...
describe("formatIssueMessage", () => {
  ...
  it("formats slot references without 'slot' prefix", () => {
    expect(
      formatIssueMessage("Issue in w1d1s1")
    ).toBe("Issue in Week 1, Day 1, Exercise 1")
  })

  it("formats equipment names after 'requires'", () => {
    expect(
      formatIssueMessage(
        "Exercise 'Lying Leg Curl' requires leg_curl_machine which is not available"
      )
    ).toBe(
  ...
  it("formats yoga_mat equipment", () => {
    expect(
      ...
      "Exercise 'Yoga Flow - Recovery' requires Yoga Mat which is not available in client's equipment list"
    )
  })
  ...
  it("handles multiple slot refs in one message", () => {
    expect(
      formatIssueMessage("Duplicate exercise in w1d1s2 and w1d2s3")
    ).toBe("Duplicate exercise in Week 1, Day 1, Exercise 2 and Week 1, Day 2, Exercise 3")
  })
})
