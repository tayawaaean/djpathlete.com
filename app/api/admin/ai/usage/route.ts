...
export async function GET() {
  try {
    ...
    const completedLogs = logs.filter(
      (l) => l.status === "completed" && l.tokens_used != null
    )

    const totalTokens = completedLogs.reduce(
      (sum, l) => sum + (l.tokens_used ?? 0),
      0
    )
    const avgTokensPerGeneration =
      completedLogs.length > 0
        ? Math.round(totalTokens / completedLogs.length)
        : 0

    const completedWithDuration = completedLogs.filter(
      (l) => l.duration_ms != null
    )
