...
function transformCsvRow(row: Record<string, string>): Record<string, unknown> {
  ...
  // Training intent — pipe-delimited or slash-delimited, e.g. "Build/Shape" or "build|express"
  if ("training_intent" in row) {
    const val = row.training_intent
    if (val && val.trim()) {
      result.training_intent = val.split(/[|/]/).map((s) => s.trim().toLowerCase()).filter(Boolean)
    } else {
      result.training_intent = ["build"]
    }
  }

  // Difficulty max (optional)
  if ("difficulty_max" in row) {
    const val = row.difficulty_max?.trim().toLowerCase()
    result.difficulty_max = val || null
  }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file with columns: name, description, category, difficulty, muscle_group, equipment, instructions, is_bodyweight, training_intent, difficulty_max, video_url. Training intent values: Build, Shape, Express (use pipe or slash to separate multiple, e.g. &ldquo;Build/Shape&rdquo;). AI metadata (movement_pattern, primary/secondary muscles, force_type, laterality, equipment_required) can be generated by Claude. Use pipe (|) to separate multiple values.
          </DialogDescription>
        </DialogHeader>
